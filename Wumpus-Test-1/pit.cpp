...
void pit::writeEffectIntoArray(int y, int x) {
...
void pit::writeEffect(int agentY, int agentX, bool gameOver) {
...
int pit::setPos(int YposC, int XposC) {
	if (YposC < pworld->getAryY() && YposC > -1 && XposC < pworld->getAryX() && XposC > -1) { //checks whether given coordinates are in ary boundaries
		if (pworld->readFieldData(YposC, XposC) == '.') {
			pworld->writeFieldData(YposC, XposC, 'P');
...
//***** Constructor *****
pit::pit(int YposC, int XposC, map* pworldC) { //Inits Gold with start coordinates and world Pointer for ary access, called in initArry() map class
	pit::Ypos = YposC;
	pit::Xpos = XposC;
	pworld = pworldC;
	pworld->writeFieldData(Ypos, Xpos, 'P');
}